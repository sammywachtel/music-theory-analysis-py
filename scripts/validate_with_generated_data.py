...
async def validate_test_cases():
    ...
    test_data_path = Path(__file__).parent.parent / 'tests' / 'generated' / 'comprehensive-modal-test-cases.json'
    ...
    for i, test_case in enumerate(sample_cases):
        ...
        try:
            ...
            if expected_modal and expected_mode:
                if is_modal:
                    ...
                    if detected_mode and expected_mode:
                        ...
                        mode_name = expected_mode.split()[-1] if ' ' in expected_mode else expected_mode
                        mode_matches = mode_name.lower() in (detected_mode or '').lower()
                else:
                    mode_matches = False
            ...
            # Progress indicator
            if (i + 1) % 10 == 0:
                print(f"  Progress: {i + 1}/{sample_size} cases processed ({passed} passed, {failed} failed)")
    ...
    if failures:
        ...
        for failure in failures[:5]:
            ...
            print(f"     Expected: {'MODAL' if failure['expected_modal'] else 'FUNCTIONAL'} ({failure['expected_mode'] or 'none'})")
            print(f"     Actual: {'MODAL' if failure['actual_modal'] else 'FUNCTIONAL'} ({failure['actual_mode'] or 'none'})")
    ...
    if failures:
        ...
        for cat, count in failure_by_category.items():
            total_in_category = len([tc for tc in sample_cases if tc['category'] == cat])
    ...
    print(f"\n🎯 OVERALL ASSESSMENT:")
    if success_rate >= 0.8:
        print(f"  🎉 EXCELLENT: {success_rate*100:.1f}% success rate - Python library shows strong behavioral parity!")
    elif success_rate >= 0.6:
        print(f"  ✅ GOOD: {success_rate*100:.1f}% success rate - Python library shows reasonable behavioral parity")
    elif success_rate >= 0.4:
        print(f"  ⚠️  MODERATE: {success_rate*100:.1f}% success rate - Some discrepancies need investigation")
    else:
        print(f"  ❌ POOR: {success_rate*100:.1f}% success rate - Significant behavioral differences detected")
