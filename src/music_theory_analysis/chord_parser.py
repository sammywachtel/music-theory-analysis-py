...
class ChordParser:
    ...
    def __init__(self):
        ...
        self.chord_templates = {
            ...
            'sus2Partial': ChordTemplate([0, 2], 'sus2(no5)', 'Suspended 2nd (no 5th)', 2, 0.75),
            'sus4Partial': ChordTemplate([0, 5], 'sus4(no5)', 'Suspended 4th (no 5th)', 2, 0.75),
            ...
            'dom7NoFifth': ChordTemplate([0, 4, 10], '7(no5)', 'Dominant 7th (no 5th)', 3, 0.80),
            'min7NoFifth': ChordTemplate([0, 3, 10], 'm7(no5)', 'Minor 7th (no 5th)', 3, 0.80),
            'maj7NoFifth': ChordTemplate([0, 4, 11], 'maj7(no5)', 'Major 7th (no 5th)', 3, 0.80),
            
            # Incomplete sus chords with extensions
            'sus2Add7': ChordTemplate([0, 2, 10], 'sus2(add7)', 'Suspended 2nd add 7th', 3, 0.75),
            'sus4Add7': ChordTemplate([0, 5, 10], 'sus4(add7)', 'Suspended 4th add 7th', 3, 0.75),
            ...
            'halfDiminished7': ChordTemplate([0, 3, 6, 10], 'm7♭5', 'Half-Diminished 7th'),
        ...
        for root_pitch in pitch_classes:
            ...
            for chord_type, template in self.chord_templates.items():
                ...
                has_all_intervals = self._check_pattern_match(
                    intervals, template.intervals, pitch_classes, 
                    root_pitch, chord_type, len(note_numbers)
                )
                ...
                if has_all_intervals:
                    ...
                    confidence = self._calculate_confidence(
                        intervals, template.intervals, len(note_numbers), 
                        chord_type, template
                    )
                    ...
                    inversion = f"/{NOTE_NAMES[bass_note]}" if bass_note != root_pitch else ''
                    
                    # Determine if partial and get pedagogical info
                    is_partial, missing_notes, completion, pedagogical = self._get_chord_info(
    ...
    def _check_pattern_match(self, intervals: List[int], template_intervals: List[int],
                            pitch_classes: List[int], root_pitch: int, 
                            chord_type: str, note_count: int) -> bool:
        """Check if intervals match template with special pattern handling"""
        
        # Basic check - all template intervals present
        has_all_intervals = all(interval in intervals for interval in template_intervals)
    ...
    def _calculate_confidence(self, played_intervals: List[int], template_intervals: List[int],
                             note_count: int, chord_type: str, template: ChordTemplate) -> float:
        """Calculate confidence score for chord match"""
        
        total_template_notes = len(template_intervals)
        matching_notes = sum(1 for interval in template_intervals if interval in played_intervals)
        extra_notes = len(played_intervals) - total_template_notes
        
        # Use predefined confidence from template if available
        confidence = template.confidence if hasattr(template, 'confidence') else (matching_notes / total_template_notes)
        
        # Adjust confidence for partial chords (2-note combinations)
        if note_count == 2 and total_template_notes == 2:
            confidence = 0.85 if matching_notes == 2 else confidence
    ...
    def _get_chord_info(self, chord_type: str, intervals: List[int], 
                       root_pitch: int, note_count: int) -> Tuple[bool, List[str], str, str]:
        ...
        if chord_type == 'sus4Partial' and len(intervals) == 2:
            ...
            missing_notes = [NOTE_NAMES[(root_pitch + 3) % 12], NOTE_NAMES[(root_pitch + 4) % 12]]
            ...
            missing_notes = [NOTE_NAMES[(root_pitch + 7) % 12]]  # Missing 5th
            completion_suggestion = f"Add {NOTE_NAMES[(root_pitch + 7) % 12]} to complete the seventh chord"
            ...
            missing_notes = [NOTE_NAMES[(root_pitch + 7) % 12]]  # Missing 5th
            completion_suggestion = f"Add {NOTE_NAMES[(root_pitch + 7) % 12]} for fuller voicing"
...
def parse_chord_progression(input_str: str) -> List[str]:
    ...
    return [chord for chord in 
            input_str.replace('|', ' ').replace(',', ' ').split() 
            if chord]
