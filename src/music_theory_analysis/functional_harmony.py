...
class FunctionalHarmonyAnalyzer:
    ...
    def _is_chord_diatonic(self, interval_from_key: int, is_minor: bool, chord_name: str) -> bool:
        """Check if chord is diatonic to the key."""
        # Diatonic intervals for each mode
        diatonic_intervals = [0, 2, 3, 5, 7, 8, 10] if is_minor else [0, 2, 4, 5, 7, 9, 11]
        ...
        expected_qualities = self._get_expected_diatonic_qualities(interval_from_key, is_minor)
        actual_quality = self._parse_chord_quality(chord_name)
        
        # Check if the actual quality matches any expected diatonic quality
        return any(self._qualities_match(actual_quality, expected) for expected in expected_qualities)
    
    def _get_expected_diatonic_qualities(self, interval_from_key: int, is_minor: bool) -> List[str]:
    ...
    def _get_roman_numeral(
        ...
        if is_chromatic:
            ...
            is_common_borrowed_chord = self._is_common_borrowed_chord(interval_from_key, is_minor)
    ...
    def _is_common_borrowed_chord(self, interval_from_key: int, is_minor: bool) -> bool:
        ...
        if is_minor:
            ...
            return interval_from_key in [1, 3, 5, 6, 8, 10]  # bii, bIII, bVII, bvi, bVI, bVII
    
    def _is_likely_secondary_dominant(self, interval_from_key: int, actual_quality: str) -> bool:
        ...
        secondary_intervals = [2, 4, 6, 8, 9, 11]  # Avoid 0, 5, 7, 10 which are often borrowed chords
    ...
    def _get_chord_function(
        ...
        return CHORD_FUNCTIONS.get(interval_from_key, {}).get(mode_key, ChordFunction.CHROMATIC)
    ...
    def _analyze_inversion_and_figured_bass(
        ...
        if chord_info.get('bass_note') is not None and chord_info['bass_note'] != chord_info['root']:
    ...
    def _identify_cadences(self, chords: List[FunctionalChordAnalysis]) -> List[Cadence]:
        ...
        for i in range(len(chords) - 1):
            ...
            elif (current.function == ChordFunction.DOMINANT and 
                  'vi' in next_chord.roman_numeral.lower()):
                cadences.append(Cadence(
                    type='deceptive',
                    chords=[current, next_chord],
                    strength=0.8,
    ...
    def _classify_progression(
        ...
        if len(chords) >= 3:
            ...
            if (ChordFunction.TONIC in functions and 
                ChordFunction.PREDOMINANT in functions and 
                ChordFunction.DOMINANT in functions):
