...
class EnhancedModalAnalyzer:
    ...
    def __init__(self):
        ...
        self.modal_patterns = [
            ...
            # Mixolydian patterns
            ModalPattern('I-bVII-IV-I', ['Mixolydian'], 0.95, PatternContext.STRUCTURAL),
            ...
            ModalPattern('I-IV-bVII-I', ['Mixolydian'], 0.88, PatternContext.STRUCTURAL),
    ...
    def analyze_modal_characteristics(self, chord_symbols: List[str], parent_key: Optional[str] = None) -> Optional[ModalAnalysisResult]:
        """
        ...
        if parent_key:
            functional_roman_numerals = self._generate_functional_roman_numerals(chord_symbols, parent_key)
            if functional_roman_numerals:
                functional_strength = self._detect_functional_patterns(functional_roman_numerals)
                if functional_strength > 0.8:
                    return None  # Block modal analysis for clear functional progressions
        ...
        foil_results = [result for result in results if self._detect_foil_patterns(result.roman_numerals)]
    ...
    def _detect_tonic_candidates(self, chord_analyses: List[ChordAnalysis], parent_key: Optional[str] = None) -> List[Dict[str, str]]:
        ...
        if parent_key:
            ...
            if parent_key_root != top_candidate:
                results.append({
                    'tonic': parent_key_root,
                    'parent_key': parent_key
                })
        
        return results[:2]  # Top 2 candidates
    
    def _analyze_with_tonic(self, chord_analyses: List[ChordAnalysis], tonic: str, 
                           parent_key: str, was_parent_key_provided: bool = True) -> Optional[ModalAnalysisResult]:
        ...
        # Collect evidence
        evidence = self._collect_evidence(chord_analyses, roman_numerals, tonic, parent_key)
        
        # Calculate confidence
        confidence = self._calculate_confidence(evidence, pattern_results, roman_numerals, chord_analyses, parent_key)
        
        # Determine mode name
        mode_name = self._determine_mode_name(pattern_results, evidence, tonic, parent_key, roman_numerals, chord_analyses)
        ...
        if is_detected_as_foil:
            final_confidence = min(confidence, 0.3)  # Reduce confidence well below threshold for foil patterns
        ...
        if not was_parent_key_provided:
            ...
            final_confidence = min(final_confidence, 0.65)  # Cap confidence for ambiguous cases
    ...
    def _generate_modal_roman_numeral(self, chord: ChordAnalysis, tonic_pitch_class: int) -> str:
    ...
    def _detect_modal_patterns(self, roman_numerals: List[str]) -> List[Dict]:
        ...
        for pattern in self.modal_patterns:
            ...
            if partial_matches > 0 and not any(r['pattern'].pattern == pattern.pattern for r in results):
                results.append({'pattern': pattern, 'matches': partial_matches})
        
        return sorted(results, key=lambda x: x['pattern'].strength * x['matches'], reverse=True)
    
    def _collect_evidence(self, chord_analyses: List[ChordAnalysis], roman_numerals: List[str], 
                         tonic: str, parent_key: str) -> List[ModalEvidence]:
        ...
        if len(chord_analyses) == 2:
            vamp_patterns = {
                'I-IV': ('I-IV vamp pattern (characteristic of Ionian modal color)', 0.7),
                'i-IV': ('i-IV vamp pattern (characteristic of Dorian modal color)', 0.8),
                'I-bVII': ('I-bVII vamp pattern (characteristic of Mixolydian modal color)', 0.85),
                'i-bII': ('i-bII vamp pattern (characteristic of Phrygian modal color)', 0.85),
                'I-II': ('I-II vamp pattern (characteristic of Lydian modal color)', 0.8)
            }
    ...
    def _generate_functional_roman_numerals(self, chord_symbols: List[str], parent_key: str) -> Optional[List[str]]:
        ...
        try:
            ...
            for chord_symbol in chord_symbols:
                try:
                    ...
                    roman = self._generate_functional_roman_numeral(chord, parent_key_pitch_class, is_minor_key)
                    result.append(roman)
                except Exception:
                    result.append('?')
    ...
    def _generate_functional_roman_numeral(self, chord: ChordAnalysis, tonic_pitch_class: int, is_minor_key: bool = False) -> str:
        ...
        expected_qualities = minor_key_qualities if is_minor_key else major_key_qualities
        ...
        mapping = next((mapping for mapping in scale_degree_mappings if mapping[0] == interval), None)
    ...
    def _detect_functional_patterns(self, roman_numerals: List[str]) -> float:
        ...
        has_modal_characteristics = any(
            'bVII' in rn or 'bII' in rn or 'II' in rn or '#IV' in rn or 'bVI' in rn or 'bIII' in rn
    ...
    def _detect_foil_patterns(self, roman_numerals: List[str]) -> bool:
        ...
        normalized_roman_numerals = [
            re.sub(r'7|maj7|m7|ø7|°7|sus|add|dim', '', rn)
            for rn in roman_numerals
        ]
        ...
        return any(pattern in [progression, normalized_progression] for pattern in modal_foil_patterns)
    
    def _calculate_confidence(self, evidence: List[ModalEvidence], pattern_results: List[Dict],
                             roman_numerals: Optional[List[str]] = None,
                             chord_analyses: Optional[List[ChordAnalysis]] = None,
                             parent_key: Optional[str] = None) -> float:
        ...
        if pattern_results:
            ...
            pattern_bonus = best_pattern['pattern'].strength * best_pattern['matches'] * 0.3
        ...
        base_confidence = evidence_strength + pattern_bonus + structural_bonus + consistency_bonus
    ...
    def _determine_mode_name(self, pattern_results: List[Dict], evidence: List[ModalEvidence],
                           tonic: str, parent_key: str, roman_numerals: List[str],
                           chord_analyses: List[ChordAnalysis]) -> str:
        ...
        if pattern_results:
            ...
            if best_pattern['pattern'].pattern == 'I-IV' and parent_key:
                ...
                interval = (self.NOTE_TO_PITCH_CLASS[tonic] - self.NOTE_TO_PITCH_CLASS[parent_root] + 12) % 12
            ...
            tonic_chords = [chord for chord in chord_analyses if chord.pitch_class == tonic_pitch_class]
            has_dominant7_tonic = any(chord.quality == 'dominant7' for chord in tonic_chords)
            has_half_diminished7_tonic = any(chord.quality == 'half_diminished' for chord in tonic_chords)
        ...
        has_natural2 = any('II' in e.description and 'bII' not in e.description for e in evidence)
        ...
        if parent_key:
            ...
            if mode_name:
                ...
                if (has_major_tonic and not has_flat7_chord and not has_flat2_chord and 
                    not has_sharp4 and not has_dominant7_tonic and not has_major7_tonic):
                    return f"{tonic} {mode_name}"
                if (has_minor_tonic and not has_major_iv and not has_minor_iv and 
                    not has_flat7_chord and not has_flat6_chord):
                    return f"{tonic} {mode_name}"
...
async def analyze_modal_progression(chords: List[str], parent_key: Optional[str] = None) -> Optional[ModalAnalysisResult]:
    """
    Analyze chord progression for modal characteristics
