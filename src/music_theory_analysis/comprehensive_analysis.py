...
from .types import UserInputContext, AnalysisOptions, MultipleInterpretationResult
...
class ComprehensiveAnalysisEngine:
    ...
    def _convert_to_multiple_interpretation_format(
        ...
        from .types import Interpretation, Evidence
        ...
        primary_interpretation = Interpretation(
            ...
            roman_numerals=[chord.roman_numeral for chord in comprehensive_result.functional.chords],
            key_signature=comprehensive_result.functional.key_signature,
            mode=comprehensive_result.modal.enhanced_analysis.mode_name if comprehensive_result.modal and comprehensive_result.modal.enhanced_analysis else None,
            evidence=[
                Evidence(
                    type='contextual',
    ...
    async def _evaluate_modal_enhancement(
        ...
        if enhanced_modal_analysis and enhanced_modal_analysis.confidence >= 0.7:
            ...
            return ModalEnhancementResult(
                ...
                when_to_use_modal=self._explain_when_to_use_enhanced_modal(enhanced_modal_analysis)
            )
        
        # Fallback to original modal detection
        has_modal_characteristics = self._detect_modal_characteristics(functional_analysis)
        
        if not has_modal_characteristics:
            return None
        
        # Create basic modal enhancement
        modal_characteristics = self._identify_modal_characteristics(functional_analysis)
        comparison_to_functional = self._compare_analytical_approaches(functional_analysis)
    ...
    def _detect_modal_characteristics(self, functional_analysis: FunctionalAnalysisResult) -> bool:
        """Detect if progression has modal characteristics."""
        if not functional_analysis.chords:
            return False
        
        roman_numerals = ' '.join(chord.roman_numeral for chord in functional_analysis.chords)
    ...
    def _identify_modal_characteristics(self, functional_analysis: FunctionalAnalysisResult) -> List[str]:
    ...
    def _compare_enhanced_analytical_approaches(
        ...
        functional_romans = ' - '.join(chord.roman_numeral for chord in functional_analysis.chords)
        modal_romans = ' - '.join(enhanced_modal_analysis.roman_numerals)
        
        return (f"Functional perspective (in {functional_analysis.key_center}): {functional_romans}. "
    ...
    def _compare_analytical_approaches(self, functional_analysis: FunctionalAnalysisResult) -> str:
    ...
    def _explain_when_to_use_enhanced_modal(self, enhanced_modal_analysis: ModalAnalysisResult) -> str:
        """Explain when enhanced modal analysis is valuable."""
        has_strong_structural = any(e.type == 'structural' and e.strength >= 0.7 for e in enhanced_modal_analysis.evidence)
        has_cadential = any(e.type == 'cadential' and e.strength >= 0.8 for e in enhanced_modal_analysis.evidence)
        
        if has_strong_structural and has_cadential:
            return (f"Modal analysis is highly recommended - the progression shows strong structural "
                   f"emphasis on {enhanced_modal_analysis.detected_tonic_center} with characteristic modal cadences")
        elif has_strong_structural:
            return (f"Modal analysis adds value - the structural pattern suggests "
    ...
    def _explain_when_to_use_modal(self, functional_analysis: FunctionalAnalysisResult) -> str:
        """Explain when modal analysis is valuable."""
        has_strong_functional = len(functional_analysis.cadences) > 0
        has_modal_chords = any(chord.is_chromatic for chord in functional_analysis.chords)
    ...
    def _analyze_chromatic_elements(self, functional_analysis: FunctionalAnalysisResult) -> Optional[ChromaticAnalysisResult]:
    ...
    def _determine_primary_approach(
        ...
        if (modal_enhancement and 
            modal_enhancement.modal_characteristics and 
            len(functional_analysis.cadences) == 0):
            return 'modal'
    ...
    def _create_comprehensive_explanation(
        ...
        if primary_approach == 'functional':
            ...
            if modal_enhancement:
                explanation += f". Modal perspective: {modal_enhancement.comparison_to_functional}"
            
            if chromatic_analysis and chromatic_analysis.secondary_dominants:
                explanation += f". Contains {len(chromatic_analysis.secondary_dominants)} secondary dominant(s)"
        
        elif primary_approach == 'modal':
            characteristics = ', '.join(modal_enhancement.modal_characteristics) if modal_enhancement else ''
            ...
            sec_dom_count = len(chromatic_analysis.secondary_dominants) if chromatic_analysis else 0
