...
from .functional_harmony import FunctionalHarmonyAnalyzer, FunctionalAnalysisResult
...
CONFIDENCE_LEVELS = {
    "definitive": {"min": 0.85, "max": 1.0, "description": "Unambiguous harmonic markers present"},
    "strong": {"min": 0.65, "max": 0.84, "description": "Clear evidence with minimal ambiguity"},
    "moderate": {"min": 0.45, "max": 0.64, "description": "Valid interpretation with competing possibilities"},
    "weak": {"min": 0.25, "max": 0.44, "description": "Theoretically possible but lacks context"},
    "insufficient": {"min": 0.0, "max": 0.24, "description": "Requires significant assumptions"}
}
...
class AnalysisCache:
    ...
    def get_cache_key(self, chords: List[str], options: Optional[AnalysisOptions] = None) -> str:
...
class MultipleInterpretationService:
    ...
    async def analyze_progression(
        ...
        try:
            # Set defaults
            pedagogical_level = PedagogicalLevel(options.pedagogical_level) if options.pedagogical_level else PedagogicalLevel.INTERMEDIATE
            confidence_threshold = options.confidence_threshold if options.confidence_threshold is not None else 0.5
            max_alternatives = options.max_alternatives if options.max_alternatives is not None else 3
            ...
            result = MultipleInterpretationResult(
                primary_analysis=ranked_interpretations[0] if ranked_interpretations else self._create_fallback_interpretation(chords),
    ...
    async def _run_functional_analysis(
        ...
        try:
            return await self.functional_analyzer.analyze_functionally(chords, options.parent_key)
    ...
    async def _run_modal_analysis(
        ...
        """Run modal analysis"""
        try:
            return self.modal_analyzer.analyze_modal_characteristics(chords, options.parent_key)
        except Exception as e:
    ...
    def _create_functional_interpretation(
        ...
        try:
            ...
            return InterpretationAnalysis(
                ...
                roman_numerals=' '.join([chord.roman_numeral for chord in functional_result.chords]),
                key_signature=functional_result.key_center or options.parent_key,
                evidence=evidence,
                reasoning=self._generate_functional_reasoning(functional_result, evidence),
    ...
    def _collect_functional_evidence(
        ...
        if functional_result.cadences:
            ...
            cadence_name = getattr(cadence, 'name', getattr(cadence, 'type', 'authentic'))
        ...
        if len(chords) >= 3 and chords[0] == chords[-1]:
            evidence.append(AnalysisEvidence(
                ...
                supported_interpretations=[InterpretationType.FUNCTIONAL, InterpretationType.MODAL],
    ...
    def _collect_modal_evidence(
        ...
        for modal_evidence in modal_result.evidence:
            chord_info = getattr(modal_evidence, 'chord', getattr(modal_evidence, 'pattern', str(modal_evidence)))
            evidence.append(AnalysisEvidence(
    ...
    def _generate_functional_reasoning(
        ...
        if functional_result.cadences:
            cadence = functional_result.cadences[0]
            cadence_name = getattr(cadence, 'name', getattr(cadence, 'type', 'authentic'))
            reasons.append(f"Strong {cadence_name} cadence establishes functional tonality")
        
        if functional_result.chords:
            reasons.append('Clear Roman numeral progression supports functional analysis')
        
        if any(e.type == EvidenceType.STRUCTURAL and e.strength > 0.6 for e in evidence):
            reasons.append('Tonic framing reinforces key center')
        
        return '; '.join(reasons) if reasons else 'Functional harmonic progression with clear tonal relationships'
    ...
    def _generate_modal_reasoning(
        ...
        if modal_result.evidence:
            ...
            chord_info = getattr(first_evidence, 'chord', getattr(first_evidence, 'pattern', str(first_evidence)))
            reasons.append(f"{chord_info} is characteristic of {modal_result.mode_name} mode")
        
        if any(e.type == EvidenceType.INTERVALLIC for e in evidence):
            reasons.append('Distinctive modal scale degrees present')
        
        return '; '.join(reasons) if reasons else f"Modal characteristics suggest {modal_result.mode_name} interpretation"
        ...
        for alt in alternatives:
            if alt.confidence >= confidence_threshold:
                alt_analysis = AlternativeAnalysis(
                    ...
                    relationship_to_primary=self._generate_relationship_description(primary, alt)
                )
    ...
    def _generate_relationship_description(
        self,
        primary: InterpretationAnalysis,
        alternative: InterpretationAnalysis
    ) -> str:
        """Generate description of relationship between interpretations"""
        if primary.type == InterpretationType.FUNCTIONAL and alternative.type == InterpretationType.MODAL:
            return 'Modal interpretation emphasizes scale degrees over functional harmonic relationships'
        elif primary.type == InterpretationType.MODAL and alternative.type == InterpretationType.FUNCTIONAL:
    ...
    def _create_fallback_interpretation(self, chords: List[str]) -> InterpretationAnalysis:
