...
class TestMultipleInterpretationService:
    ...
    async def test_modal_primary_analysis(self):
        ...
        assert len(result.alternative_analyses) > 0 or result.primary_analysis.type == InterpretationType.MODAL
        ...
        if result.primary_analysis.type == InterpretationType.MODAL:
            ...
            modal_analysis = next((alt for alt in result.alternative_analyses 
                                 if alt.type == InterpretationType.MODAL), None)
        
        if modal_analysis:
            assert modal_analysis.mode is not None
            assert any(ev.type == EvidenceType.INTERVALLIC for ev in modal_analysis.evidence)
    ...
    async def test_alternative_analyses_filtering(self):
        ...
        assert len(low_result.alternative_analyses) >= len(high_result.alternative_analyses)
    ...
    async def test_caching_behavior(self):
        ...
        assert result1.primary_analysis.confidence == result2.primary_analysis.confidence
    ...
    async def test_metadata_completeness(self):
        ...
        assert metadata.pedagogical_level in [PedagogicalLevel.BEGINNER, 
