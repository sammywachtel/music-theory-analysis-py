...
class TestChromaticAnalysis:
    ...
    def test_secondary_dominant_detection(self):
        """Test secondary dominant detection and resolution"""
        # Test progression with V/V (D7 in C major)
        chords = ['C', 'D7', 'G', 'C']
        functional_result = self.functional_analyzer.analyze_functionally(chords, 'C major')
        
        chromatic_result = self.chromatic_analyzer.analyze_chromatic_elements(functional_result)
        ...
        assert 'V/V' in sec_dom.roman_numeral or 'V/iv' in sec_dom.roman_numeral
        assert sec_dom.target in ['V', 'iv']
    ...
    def test_multiple_secondary_dominants(self):
        ...
        if chromatic_result:
            ...
            complexity = self.chromatic_analyzer.get_chromatic_complexity_score(chromatic_result)
            assert complexity > 0.2
    
    def test_borrowed_chord_detection(self):
    ...
    def test_no_chromatic_elements(self):
        ...
        # Should return None for pure functional progressions
        assert chromatic_result is None
    
    def test_chromatic_complexity_scoring(self):
    ...
    def test_should_lead_with_chromatic(self):
        ...
        assert self.chromatic_analyzer.should_lead_with_chromatic_analysis(result_with_sec_dom)
        ...
        assert not self.chromatic_analyzer.should_lead_with_chromatic_analysis(result_with_borrowed)
        ...
        assert self.chromatic_analyzer.should_lead_with_chromatic_analysis(result_with_mediant)
    ...
    def test_secondary_dominant_chains(self):
        ...
        chains = self.chromatic_analyzer.analyze_secondary_dominant_chains(secondary_dominants)
    ...
    def test_borrowed_chord_patterns(self):
        ...
        patterns = self.chromatic_analyzer.analyze_borrowed_chord_patterns(borrowed_chords)
    ...
    def test_chromatic_explanation_generation(self):
        ...
        mock_result = ChromaticAnalysisResult(
            ...
            chromatic_mediants=[],
            resolution_patterns=[
                ResolutionPattern('V/V', 'V', ResolutionType.STRONG, 'Strong resolution pattern')
            ]
        )
        
        explanation = self.chromatic_analyzer.generate_chromatic_explanation(mock_result)
        
        # Should mention secondary dominants
        assert 'secondary dominant' in explanation.lower()
        # Should mention borrowed chords
        assert 'borrowed' in explanation.lower() or 'modal interchange' in explanation.lower()
    ...
    def test_convenience_function(self):
        ...
        direct_result = self.chromatic_analyzer.analyze_chromatic_elements(functional_result)
        
        if chromatic_result and direct_result:
            assert len(chromatic_result.secondary_dominants) == len(direct_result.secondary_dominants)
...
class TestChromaticIntegration:
    ...
    def test_functional_to_chromatic_pipeline(self):
    ...
    def test_jazz_progression_chromatic_analysis(self):
        ...
        if chromatic_result:
            ...
            should_lead = self.chromatic_analyzer.should_lead_with_chromatic_analysis(chromatic_result)
            # May or may not lead depending on what's detected
            assert isinstance(should_lead, bool)
    
    def test_modal_interchange_in_major(self):
        ...
        if chromatic_result and chromatic_result.borrowed_chords:
            ...
            patterns = self.chromatic_analyzer.analyze_borrowed_chord_patterns(chromatic_result.borrowed_chords)
            assert len(patterns) >= 0  # May detect patterns
    
    def test_chromatic_analysis_confidence(self):
