...
class TestEnhancedModalAnalyzer:
    ...
    def test_dorian_detection(self):
        ...
        result = self.analyzer.analyze_modal_characteristics(['Am', 'D', 'Am'], 'C major')
    ...
    def test_phrygian_detection(self):
        ...
        result = self.analyzer.analyze_modal_characteristics(['Em', 'F', 'Em'], 'C major')
    ...
    def test_mixolydian_with_dominant7(self):
        ...
        result = self.analyzer.analyze_modal_characteristics(['G7', 'F', 'G7'], 'C major')
    ...
    def test_evidence_collection(self):
        ...
        result = self.analyzer.analyze_modal_characteristics(['G', 'F', 'C', 'G'], 'C major')
        
        assert result is not None
        assert len(result.evidence) > 0
        
        # Check for structural evidence (starts and ends on same chord)
        structural_evidence = [e for e in result.evidence if e.type == EvidenceType.STRUCTURAL]
        assert len(structural_evidence) > 0
        
        # Check for intervallic evidence (bVII chord)
        intervallic_evidence = [e for e in result.evidence if e.type == EvidenceType.INTERVALLIC]
    ...
    def test_parent_key_context(self):
        ...
        assert result1.mode_name != result2.mode_name or result1.confidence != result2.confidence
    ...
    def test_roman_numeral_generation(self):
        ...
        for chord_symbol, quality, expected_roman in test_cases:
            chord_analysis = self.analyzer._parse_chord(chord_symbol)
            roman = self.analyzer._generate_modal_roman_numeral(chord_analysis, 0)  # C = pitch class 0
